...
def tez_denetle(file_stream, progress_bar=None, progress_text=None):
    ...
    for i, para in enumerate(doc.paragraphs):
        ...
        is_quote_paragraph = text.startswith(('"', '“', '«', "''"))
        ...
        if has_invalid_font:
            if is_quote_paragraph:
                hata_mesaji += " [HATA: Alıntı metinlerin yazı tipi Arial olmalıdır!]"
            else:
                hata_mesaji += " [HATA: Yazı tipi Times New Roman olmalıdır!]"
        if has_invalid_size:
            if is_quote_paragraph:
                hata_mesaji += " [HATA: Alıntı metinlerin yazı boyutu 10 pt olmalıdır!]"
            else:
        ...
        if not kapak_modu:
            ...
            if tek_satir_modu or style_name in TEK_SATIR_STILLER:
            ...
            elif kaynakca_modu or style_name in ANA_METIN_STILLER or is_quote_paragraph or effective_spacing is not None:
                # ANA METİN / KAYNAKÇA / ALINTI (1.5) bölgesi
                ...
                if not text.startswith(("Şekil", "Tablo", "Resim", "Not.")):
                    if effective_spacing is not None and (
                        ...
                        if is_quote_paragraph:
                            hata_mesaji += (
                                f" [HATA: Alıntı bölümlerinde satır aralığı 1.5 olmalı! "
                                f"(Algılanan: {effective_spacing:.2f})]"
                            )
                        else:
                            hata_mesaji += (
...
st.title("🎓 Balıkesir Üniversitesi Tez Denetim Sistemi (v35 – R12)")
...
| Bölüm | Kural |
|---|---|
| Ana metin, Kaynakça, Alıntılar | 1,5 satır aralığı |
| Kabul ve Onay, Etik Beyan, Özet, Abstract, İçindekiler, Şekil/Tablo/Sembol/Kısaltma Listesi, Önsöz, Teşekkür | Tek satır (1,0) |
| Şekil/Tablo açıklamaları, dipnotlar | Tek satır — *stil tabanlı, otomatik muaf* |
""")
