...
def tez_denetle(file_stream):
    ...
    for para in doc.paragraphs:
        ...
        # ── ALFABETİK SIRA (KAYNAKÇA) ───────────────────────────────────
        # Kaynakça içinde kaynak olmayan satırları (devam başlıkları, ekler vb.) atla
        KAYNAK_DISI_ONEKLER = (
        ...
        if kaynakca_modu and len(text) > 3 and not kaynak_disi:
            ...
            if current_val and son_kaynak:
                # Türk alfabesi sırasına göre karşılaştır (Python Unicode değil)
...
st.title("🎓 Balıkesir Üniversitesi Tez Denetim Sistemi (v29 – R10)")
